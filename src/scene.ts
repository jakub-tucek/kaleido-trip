...
export class VisualizerScene {
  ...
  constructor(canvas: HTMLCanvasElement, private readonly controls: SceneControls) {
    ...
    this.scene.fog = new THREE.FogExp2(0x04030a, 0.082);

    this.camera = new THREE.PerspectiveCamera(55, window.innerWidth / window.innerHeight, 0.1, 200);
    this.camera.position.set(0, 0.6, 7.5);

    this.scene.add(new THREE.AmbientLight(0x4f456b, 0.35));

    this.frontLight = new THREE.PointLight(0xb14567, 5.5, 28, 2);
    this.frontLight.position.set(0, 1.5, 3.2);
    this.scene.add(this.frontLight);

    this.backLight = new THREE.PointLight(0x4b4f8f, 4.5, 34, 2);
    this.backLight.position.set(0, -0.8, -6.2);
    this.scene.add(this.backLight);

    this.rimLight = new THREE.PointLight(0xcdd6ff, 1.8, 20, 2);
    ...
    for (let i = 0; i < 128; i += 1) {
      const material = new THREE.MeshBasicMaterial({
        color: new THREE.Color().setHSL(0.9 - i / 620, 0.38, 0.34),
        transparent: true,
        opacity: 0.28,
      });
  ...
  render(profile: Profile, time: number, pointer: PointerState, frequencyData?: Uint8Array, waveformData?: Uint8Array, isLive = false): void {
    const bloom = this.controls.getBloom();

    this.renderer.setClearColor(new THREE.Color().setHSL(0.77 + profile.highs * 0.03, 0.2, 0.02 + bloom * 0.025), 1);
    this.frontLight.intensity = 1.6 + bloom * 2.8 + profile.level * 4.8;
    this.backLight.intensity = 1.2 + bloom * 2.1 + profile.highs * 3.5;
    this.rimLight.intensity = 0.6 + bloom * 0.9 + profile.highs * 0.8;
  ...
  private updateCore(profile: Profile, time: number, bloom: number): void {
    ...
    this.coreMaterial.emissiveIntensity = 0.12 + bloom * 0.6 + profile.level * 1.3;
    this.coreMaterial.color.setHSL(0.88 - profile.highs * 0.08, 0.2, 0.05 + profile.level * 0.04);
  ...
  private updateRings(profile: Profile, time: number): void {
    this.haloRings.forEach((ring, index) => {
      ...
      ring.material.opacity = 0.025 + profile.level * 0.08 + index * 0.006;
  ...
  private updateBars(profile: Profile, time: number, frequencyData?: Uint8Array, isLive = false): void {
    this.bars.forEach((bar, index) => {
      ...
      bar.material.opacity = 0.12 + value * 0.45;
      bar.material.color.setHSL(0.92 - value * 0.12 - index / 1000, 0.45, 0.26 + value * 0.12);
    });
  ...
  private updateRibbon(profile: Profile, time: number, waveformData?: Uint8Array, isLive = false): void {
    ...
    for (let i = 0; i < positions.length; i += 3) {
      ...
      const wave = isLive && waveformData
        ...
        : Math.sin(time * 0.004 + x * 0.8 + yIndex * 0.15) * profile.level;
    ...
    this.ribbonMaterial.opacity = 0.08 + profile.level * 0.14;
    this.ribbonMaterial.color.setHSL(0.74 + profile.highs * 0.08, 0.32, 0.22 + profile.level * 0.08);
  }
  ...
  private updateTunnel(profile: Profile, time: number): void {
    ...
    this.tunnelMaterial.opacity = 0.18 + profile.level * 0.42;
    this.tunnelMaterial.color.setHSL(0.86 - profile.highs * 0.08, 0.48, 0.58);
  }
  ...
  private updateWaveLines(profile: Profile, time: number, waveformData?: Uint8Array, isLive = false): void {
    this.waveformLines.forEach((line, lineIndex) => {
      ...
      for (let i = 0; i < points; i += 1) {
        ...
        const wave = isLive && waveformData
          ? (waveformData[waveformIndex] - 128) / 128
          : Math.sin(time * 0.005 + xNorm * 18 + lineIndex * 0.25) * profile.level;
      ...
      line.material.opacity = 0.025 + profile.level * 0.12 + lineIndex * 0.002;
    });
  }
